...
for m=1:L-2
    ...
    for i=1:4
        if stafind(i,1)<=4&&stafind(i,2)<=4
            if distance(Y1(stafind(i,1),1:2*(2+m)),X(1:2*(2+m)))>=distance(Y1(stafind(i,2),1:2*(2+m)),X(1:2*(2+m)))
                M1(stafind(i,1),1)=0;
            else
                M1(stafind(i,2),1)=0;
            end
        elseif stafind(i,1)>4&&stafind(i,2)>4
            if distance(Y2(stafind(i,1)-4,1:2*(2+m)),X(1:2*(2+m)))>=distance(Y2(stafind(i,2)-4,1:2*(2+m)),X(1:2*(2+m)))
                M2(stafind(i,1)-2^2,1)=0;
            else
                M2(stafind(i,2)-2^2,1)=0;
            end
        elseif stafind(i,1)<=4&&stafind(i,2)>4
            if distance(Y1(stafind(i,1),1:2*(2+m)),X(1:2*(2+m)))>=distance(Y2(stafind(i,2)-4,1:2*(2+m)),X(1:2*(2+m)))
                M1(stafind(i,1),1)=0;
                e2se
                M2(stafind(i,2)-2^2,1)=0;
            end
        elseif stafind(i,1)>4&&stafind(i,2)<=4
            if distance(Y2(stafind(i,1)-4,1:2*(2+m)),X(1:2*(2+m)))>=distance(Y1(stafind(i,2),1:2*(2+m)),X(1:2*(2+m)))
